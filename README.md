...
```bash
conda create python=3.8 -n goalstack
```
2 - <a name="step-2">Activate the conda environment</a>

```bash
conda activate goalstack
```
...
If for some reason you get an error similar to bellow, is because the DATABASE_URL is configured to `postgres:///goalstack` and because of it the generated `DATABASES` settings are configured to connect on PostgreSQL using the socket mode.
...
LINE 1: CREATE DATABASE goalstack WITH ENCODING 'UTF-8' OWNER "";
                                                             ^
ERROR:  syntax error at or near ";"
LINE 1: GRANT ALL PRIVILEGES ON DATABASE goalstack TO ;
```
...
ERROR:  database "goalstack" already exists
GRANT
```
...
```bash
sudo -u postgres dropdb -U postgres --if-exists goalstack
